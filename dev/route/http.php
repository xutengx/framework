...
//		return obj(\Gaara\Core\Response::class)->setContentType('html')->setContent('hello world');
	...
	// yh
	Route::group(['middleware' => ['web'], 'namespace' => 'Apptest\yh\c'], function() {
		// 接口开发调试页面
	...
	Route::group(['middleware' => ['web', 'api'], 'namespace' => 'Apptest\Dev'], function() {
		// idehelp 页面
		...
		Route::group(['prefix' => 'performance', 'namespace' => 'performance\Contr'], function() {
			...
			Route::get('/{class}/{func}', function($class, $func) {
				return obj('Apptest\Dev\performance\Contr\\' . $class . 'Contr')->$func();
			});
		});
	...
	// 管道模式
	Route::get('/p', ['middleware' => ['web', 'testMiddleware'], 'namespace' => 'Apptest\Dev', 'uses' => 'Pipeline\index@index']),
